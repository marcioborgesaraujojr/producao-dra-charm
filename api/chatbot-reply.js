...
function montarSystem(cfg, cliente) {
...
async function viaOpenAI(cfg, mensagens, cliente) {
  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + process.env.OPENAI_API_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      model: cfg.modelo || 'gpt-4o-mini',
      messages: [{ role: 'system', content: montarSystem(cfg, cliente) }, ...mensagens],
...
export default async function handler(req, res) {
  ...
  try {
    let reply;
    if (usaClaude && temAnthropic) reply = await viaAnthropic(cfg, mensagens, cliente);
    else if (temOpenAI) reply = await viaOpenAI(cfg, mensagens, cliente);
    else if (temAnthropic) reply = await viaAnthropic(cfg, mensagens, cliente);
