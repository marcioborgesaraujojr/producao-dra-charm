...
//
// Só LEITURA, e de um arquivo público. Nunca escreve nada.
...
function resumirProduto(p) {
  const tam = (Array.isArray(p.tamanhos) ? p.tamanhos : [])
    ...
    .map(t => ({ tamanho: t.tamanho, tem: Number(t.saldo) > 0 }));
  return {
    nome: p.nome,
...
export async function procurarNoEstoque(texto, limite = 4) {
  try {
    ...
    const lista = finalistas.slice(0, limite).map(a => resumirProduto(a.p));
    return { termos, produtos: lista, atualizado_em: dados.atualizado_em || null, achadosNoTotal: finalistas.length };
  } catch (e) {
    return null;
  }
}
