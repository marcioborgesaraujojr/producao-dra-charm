...
export function estoqueEmTexto(r) {
  ...
  if (r.combinacaoNaoExiste) {
    ...
    /* O texto de "não fazemos sob encomenda" é dele, palavra por palavra (21/08). A versão
       anterior — "Não, a gente não faz sob encomenda. Só o que está no site" — ele achou
       seca: dá a notícia ruim sem dizer por quê nem mostrar o que a cliente ganha com isso. */
    l.push('COMO RESPONDER: diga que nesse modelo não tem essa cor, e mande a busca do MODELO pra ela ver '
      ...
      + 'Se ela pedir pra mandar fazer na cor que não tem, a resposta é no espírito desta (sem decorar): '
      + '"infelizmente não conseguimos fazer sob encomenda por causa da nossa logística de produção e '
      + 'entrega. Todas as peças do site já são a pronta entrega, pra sua compra chegar o mais rápido '
      + 'possível." Explique o motivo — nunca corte com um "não" seco.');
    return l.join('\n');
  }
