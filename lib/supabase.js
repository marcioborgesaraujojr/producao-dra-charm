...
// Conta linhas de forma exata via header Content-Range (não puxa as linhas).
export async function count(table, where = '') {
  let url = `${table}?select=id`;
  if (where) url += `&${where}`;
  const res = await fetch(`${URL}/rest/v1/${url}`, { method: 'HEAD', headers: headers({ Prefer: 'count=exact', Range: '0-0' }) });
