...
function auth() {
  return `chave_api ${process.env.LI_CHAVE_API} chave_aplicacao ${process.env.LI_CHAVE_APLICACAO}`;
}
async function liFetch(path, opts = {}) {
  ...
  if (!res.ok) throw new Error(`LI ${res.status} ${path}: ${(await res.text().catch(() => '')).slice(0, 200)}`);
...
export async function fetchOrders() {
  ...
  const data = await liFetch('/pedido?limit=50');
