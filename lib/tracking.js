...
// Motoboy: sem rastreio (tratado por regra). Mock quando faltam credenciais.
...
export async function track(order) {
  ...
  try {
    // entrega local/própria ("Personalizada", motoboy): sem rastreio externo (tratado por regra)
    if (carrier === 'local' || carrier === 'motoboy') return { found: false, events: [] };
    if (carrier === 'melhorenvio') return await trackMelhorEnvio(order);
    if (carrier === 'jt') return await trackJT(order);
...
// ---------------- J&T Express ----------------
async function trackJT(order) {
  const key = process.env.JT_API_KEY;
  if (!key || !order.tracking_code) return mockCorreios(order); // demo/sem credencial
  // Endpoint conforme seu contrato J&T. Estrutura comum abaixo.
  const res = await fetch(`https://api.jtexpress.com.br/tracking/${order.tracking_code}`, {
    headers: { 'x-api-key': key },
  });
  if (!res.ok) throw new Error(`J&T ${res.status}`);
  const data = await res.json();
  const events = (data.details || data.traces || []).map((e) => ({
    data: e.scanTime || e.time || '', status: e.scanType || '',
    descricao: e.desc || e.description || '', local: e.city || '',
  }));
...
async function trackCorreios(order) {
  ...
  if (!token) return mockCorreios(order); // sem credenciais -> demonstração
...
async function trackMelhorEnvio(order) {
  ...
  if (!token) return mockCorreios(order);
